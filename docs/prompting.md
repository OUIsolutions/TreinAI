...
(you dont need to especify the hole path, just the file name)
...
with it , it will get a better knolage about the project

## Memory

### Saving memory Manually
what ever file you put inside **TreinAiContext** it will be available as a memory, so TreinAI will proritize the files that are in the memory

### Asking to memorize somethin
you can ask the TreinAi to memorize a a action it did, for example, if you want to memorize the action of reading the file `example.txt`, you can ask the TreinAi to do so by typing `memorize read example.txt`.
